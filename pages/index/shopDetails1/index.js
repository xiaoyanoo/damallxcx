// pages/index/shopDetails/index.js
//获取应用实例
const app = getApp()
...
Page({
  ...
  linkOrderPay: function () {
    ...
    //配送的类型
    var deliver_time = that.data.deliver_time;
    var deliver_time_type = that.data.deliver_time_type;
    var data = {
      goods_id: ids,
      goods_num: skuNum,
      goods_spec: sku_num,
  ...
  chakan:function(e){
    ...
    wx.navigateTo({
      url: '../shopDetails/index?ids=' + goods_id,
    })
  },
  //输入框输入文字
  getInfo: function (e) {
  ...
  //切换配送方式
  tebdeliver: function(e){
    var index = e.currentTarget.dataset.index; 
    this.setData({
      deliver_time_type: index
    })
    if(index == 1){
      this.setData({
        deliver_time: ''
  ...
  getDetails: function () {
    ...
    var ids = that.data.ids;
    var user_id = that.data.user_id;
    var getDetailsUrl = jiekou + '/WXAPI/Homepage/preGoodsInfo';
    wx.request({
      url: getDetailsUrl,
      data: {
        goods_id: ids,
  ...
  selectSpec: function (e) {
    ...
    //根据规格获取价格和库存
    var getspecUrls = jiekou + '/WXAPI/Homepage/preSalePrice';
    wx.request({
      url: getspecUrls,
      data: {
        id: specArr.join('_')
